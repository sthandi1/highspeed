...
def fft_testing(filename):
    ...
    normalised_jet_diameter_freqs = rfftfreq(len(normalised_jet_diameter),
                                             1/27000)
    fig6, ax6 = plt.subplots()
    ax6.stem(normalised_jet_diameter_freqs,
             np.abs(normalised_jet_diameter_fft))
    ...
    normalised_jet_centroid_freqs = rfftfreq(len(normalised_jet_centroid),
                                             1/27000)
    fig8, ax8 = plt.subplots()
    ax8.stem(normalised_jet_centroid_freqs,
             np.abs(normalised_jet_centroid_fft))
    ax8.set_title('Shifted jet centroid FFT')
    ax8.set_xlabel('Frequencies')
    ax8.set_ylabel('Amplitude')
    ax8.set_xlim(0, 250)

    print(len(normalised_jet_centroid_freqs))
